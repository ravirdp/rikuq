...
### Exact caching (Redis fingerprint) — <5% hit rate, 1-3% savings
...
3. **Low-volume workloads (<10K calls/month).** The engineering cost of setting up semantic/exact caching layers exceeds the savings. Use Anthropic native caching only (zero engineering cost) and skip the rest.
